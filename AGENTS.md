...
- Python: .venv/bin/python (uv, CPython 3.14.x)
...
- [Merge gates](.github/CONTRIBUTING.md#merge-gates) — CI green +
  review findings addressed + `mergeable=CLEAN` +
...
actual GitHub state (status check rollup, review submissions,
`mergeable` field) rather than asserting them from local history.
Local `uv run pytest` / `uv run ruff` runs are encouraged but not substitutes for the cloud gates.
...
- OpenSpec is a hard gate for behavior, API, schema, CLI,
  ...
  required.
- Review state must come from current-head GitHub evidence. Check labels,
  latest review submissions/comments/reactions, and GraphQL review threads before
  claiming approval. Missing review evidence is not approval. Unresolved
  non-outdated blocking threads block readiness even when a top-level review
  comment looks clean.
