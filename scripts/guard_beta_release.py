...
RELEASE_MANAGED_FILES = (
    ...
    "deploy/helm/firecrawl-lb/Chart.yaml",
    "uv.lock",
)
...
_VALIDATION_HEADING_RE = re.compile(r"^\s{0,3}#{2,6}\s+Release-candidate validation\s*#*\s*$", re.IGNORECASE)
_MARKDOWN_HEADING_RE = re.compile(r"^\s{0,3}#{1,6}\s+")
_VALIDATED_CANDIDATE_RE = re.compile(r"^\s*Validated candidate:\s*([0-9a-f]{40})\s*$", re.IGNORECASE | re.MULTILINE)
...
def _read_project_versions_at_ref(root: Path, ref: str) -> dict[str, str]:
    ...
    chart_text = _read_ref_text(root, ref, "deploy/helm/firecrawl-lb/Chart.yaml")
    ...
    return {
        ...
        "deploy/helm/firecrawl-lb/Chart.yaml version": find(r"^version: (.+)$", chart_text, "chart version"),
        "deploy/helm/firecrawl-lb/Chart.yaml appVersion": find(r"^appVersion: (.+)$", chart_text, "chart appVersion"),
        "uv.lock": find(
            r'\[\[package\]\]\nname = "firecrawl-lb"\nversion = "([^"]+)"\nsource = \{ editable = "\." \}',
            uv_text,
            "uv.lock firecrawl-lb version",
        ),
    }
...
def _release_candidate_validation_section(body: str) -> str | None:
    lines = body.splitlines()
    for index, line in enumerate(lines):
        if not _VALIDATION_HEADING_RE.match(line):
            continue
        section: list[str] = []
        for section_line in lines[index + 1 :]:
            if _MARKDOWN_HEADING_RE.match(section_line):
                break
...
def require_validation_evidence(body: str, expected_sha: str) -> None:
    missing: list[str] = []
    validation_section = _release_candidate_validation_section(body)

    if validation_section is None:
        missing.append("a `Release-candidate validation` section")

    validated_candidate_match = _VALIDATED_CANDIDATE_RE.search(validation_section or "")
    validated_candidate = validated_candidate_match.group(1).lower() if validated_candidate_match is not None else ""
    if validated_candidate != expected_sha.lower():
...
def guard_pull_request(root: Path, event: dict[str, Any], base_ref: str, head_ref: str) -> None:
    ...
    is_merge_group = event_name == "merge_group"
    if pr is None and event_name and not is_merge_group:
    ...
    if pr is None and is_merge_group:
        missing_evidence: list[str] = []
        if not head_ref:
            missing_evidence.append("head branch evidence via --head-ref or GITHUB_HEAD_REF")
        if not body:
            missing_evidence.append("validation body evidence via BETA_RELEASE_PR_BODY")
        if missing_evidence:
            detail = "\n".join(f"- {item}" for item in missing_evidence)
            raise GuardError(
                "merge_group beta release guard detected beta release metadata but cannot validate the source PR.\n"
