...
def event_file(
    ...
    *,
    ...
    head_repo: str = "Soju06/firecrawl-lb",
    base_repo: str = "Soju06/firecrawl-lb",
) -> Path:
...
def test_pr_guard_rejects_stale_validated_candidate_line_with_expected_sha_elsewhere(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    sha = init_repo_with_beta_commit(repo)
    branch = "release/beta-1.20.0-beta.3"
    stale_sha = "0" * 40
    body = f"""## Summary
This body mentions the current head elsewhere: {sha}

## Release-candidate validation
Validated candidate: {stale_sha}
...
"""
    ...
    assert result.returncode == 1
    assert "the exact validated candidate SHA" in result.stderr
    assert sha in result.stderr
...
def test_pr_guard_rejects_merge_group_beta_metadata_without_pr_evidence(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    init_repo_with_beta_commit(repo)
    event = tmp_path / "merge_group_event.json"
    event.write_text(json.dumps({"repository": {"full_name": "Soju06/firecrawl-lb"}}), encoding="utf-8")
    ...
    result = run_guard(
        ...
        "--event-path",
        str(event),
        env_overrides={"GITHUB_EVENT_NAME": "merge_group"},
    )

    assert result.returncode == 1
    assert "merge_group beta release guard detected beta release metadata" in result.stderr
    assert "Expected head branch: release/beta-1.20.0-beta.3" in result.stderr
    assert "head branch evidence" in result.stderr
    assert "validation body evidence" in result.stderr
...
def test_pr_guard_rejects_forked_canonical_beta_branch(tmp_path: Path) -> None:
    ...
    event = event_file(
        ...
        head_repo="evil-fork/firecrawl-lb",
    )
    ...
    assert "Expected head repository: Soju06/firecrawl-lb" in result.stderr
    assert "Actual head repository: evil-fork/firecrawl-lb" in result.stderr
